...
def _callback_scan(classinstance, msgtype, note_cc, mess): #class instance , messagetype that triggered 
    '''    
    The mess parameter contains everything
...
session=Session(mido)

def main():
    
    #### I'm starting to move code into the session object ....
    # global midi, ports
    # global sd
    ...
    # session.new(ports, midi, filterMidi)
    
    # session._addChordScheme("")
    # session._addControlScheme("")
    # session._addUserData(SessionData) #smaller things that are used during session, index, channel, ...
    ...
    session.addMidiChannels(md)
    session.userinfo() ## all that print something , collect it here ...
    ...
    filterMidi.setmidichannelBass(default_midi_channel_in_bass)
    filterMidi.setmidichannelChordControl(default_midi_channel_in_control_chords)
    filterMidi.report()
    
    print(midi.printMidiChannels())     
    
    #session.destruct(midi, ports)
    midi.startLoop_keyboardlistener(session.destruct)# polling the keyboard 
